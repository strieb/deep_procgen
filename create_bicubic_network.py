...
def create_network(size, map, mask, seed, output_size):
    with tf.name_scope("stride_network"):
        ...
        random_0 = stateless_random_uniform((1, size, size, 4), [seed,1])
        in_all = tf.concat([map, mask,random_0], -1)
        ...
        layer_1_resize = tf.image.resize_bilinear(layer_2_b, (size_1, size_1), align_corners=True)
        layer_1_conc = tf.concat([layer_1_resize, input_layer_1_b], -1)
        layer_1_b = tf.layers.conv2d(layer_1_conc, 16, (3, 3),  activation=tf.nn.relu, padding="SAME")

        layer_0_resize = tf.image.resize_bilinear(layer_1_b, (size, size), align_corners=True)
        layer_0_conc = tf.concat([layer_0_resize, input_layer_0_b], -1)
        layer_0_b = tf.layers.conv2d(layer_0_conc, 32, (5, 5),  activation=tf.nn.relu, padding="SAME")
        layer_0_b = tf.layers.conv2d(layer_0_b, 32, (5, 5),  activation=tf.nn.relu, padding="SAME")
